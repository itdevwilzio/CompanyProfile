<nav class="w-full bg-white py-4">
    ...
    <ul class="flex flex-wrap navbar-mobile lg:navbar-desktop transition-all z-[999] mx-auto" id="navbar">
        <button id="close-navbar-mobile"
            class="h-8 border border-red-400 bg-red-700 px-2 -mr-5 -mt-5 flex items-center justify-center text-white self-end rounded mb-10 lg:hidden shadow-[0_4px_0_rgba(0,0,0,0.4)] hover:shadow-[0_2px_0_rgba(0,0,0,0.4)] active:shadow-[0_1px_0_rgba(0,0,0,0.6)] hover:translate-y-1 active:translate-y-2 transition-all duration-300 ease-in-out hover:bg-yellow-500">
            <i class="feather icon-x me-1"></i> Close
        </button>
        <li
            class="font-roboto font-bold text-base leading-6 transition-all duration-300 hover:text-orange-500 hover:text-xl {{ request()->routeIs('front.index') ? 'lg:bg-white bg-cp-black lg:text-cp-dark-blue' : '' }}">
            <a href="{{ route('front.index') }}">Beranda</a>
        </li>
    
        <!-- Dropdown for Produk -->
        <li class="font-roboto font-bold transition-all duration-300 nav-item has-sub relative hover:text-orange-500 hover:text-xl">
            <button id="dropdownToggle" class="font-roboto font-bold transition-all duration-300 click:text-cp-dark-blue focus:outline-none">
                Produk
            </button>
            <div id="dropdownMenu" class="hidden lg:absolute w-full lg:w-fit mt-4 lg:mt-0 text-white lg:text-gray-900 lg:bg-white sub-item lg:left-1/2 lg:-translate-x-1/2 lg:shadow-lg">
                <ul class="flex flex-col grow text-left">
                    <li
                        class="font-roboto font-bold text-base transition-all text-nowrap p-2 duration-300 hover:text-orange-500 hover:text-xl child-item  {{ request()->routeIs('front.product') ? 'lg:bg-white bg-cp-black lg:text-cp-dark-blue' : '' }}">
                        <a href="{{ route('front.product') }}">Paket Home</a>
                    </li>
                    <li
                        class="font-roboto font-bold text-base transition-all text-nowrap p-2 duration-300 hover:text-orange-500 hover:text-xl child-item  {{ request()->routeIs('front.location') ? 'lg:bg-white bg-cp-black lg:text-cp-dark-blue' : '' }}">
