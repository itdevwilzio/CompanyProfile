...
@section('content')
    ...
    {{-- Section Voucher --}}
    <div class="container mx-auto flex flex-col mt-20 sm:mt-10 max-w-2xl text-white p-2">
        @if (session()->has('success'))
            <div class="my-5 p-4 bg-green-500 text-white rounded-lg">{{ session('success') }}</div>
        @endif
        <h1 class="text-yellow-400 font-bold text-xl text-center mb-10 sm:mb-6">Beli Voucher Wi-Fi</h1>

        {{-- Display Location and Voucher Information --}}
        <div class="text-center mt-10 sm:mt-5 px-4 md:px-0">
            <div class="mb-4">
                <img src="{{ asset($location->image) }}" alt="{{ $location->name }}" class="mx-auto rounded-lg w-48 h-48 object-cover">
            </div>
            <h2 class="text-xl font-semibold">{{ $location->name }}</h2>
            <h3 class="text-lg mt-4 font-semibold text-yellow-300">{{ $voucher->name }}</h3>
            <p class="text-lg font-bold text-green-400">Price: ${{ $voucher->price }}</p>

            {{-- WhatsApp Button Below Text --}}
            <a href="https://wa.me/6285179706639?text=Halo%20CSP%20Wilzio"
...
@endsection
    ...
    <footer class="w-full mt-10 sm:mt-6 md:mt-8 bg-[#0E3995] text-white">
        <div class="container max-w-7xl mx-auto flex flex-col items-center justify-center py-10 sm:py-8 md:py-10">
            <!-- Follow Us Section -->
            <p id="CompanyName" class="font-nunito font-bold text-lg mb-4">Ikuti Kami</p>
            <div class="flex items-center gap-6 mb-6">
                <!-- Social Media Icons with Hover Effect -->
                <a href="https://www.facebook.com/official.wilzio" target="_blank">
                    <div
